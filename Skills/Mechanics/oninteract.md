## 描述
Applies an 光环 to a PLAYER that 触发 a skill when they interact (right click) while holding an item, or are looking at a block (NOT AIR) that gets outlined (5 block range).

Applies an 光环 to an entity that 触发 a skill when they are interacted with (gets right clicked) by a player


## 属性
| 属性 | 缩写   | 描述                                                          | 默认值 |
|-----------|-----------|----------------------------------------------------------------------|---------|
| onrightclickskill | oninteractskill, onrightclick, oninteract, or, oi | Skill to 执行 if the 目标 player interacts with a block or the 目标 entity is interacted by a player                   |<!--type:Metaskill-->|

> This 技能 继承 [光环] 技能


## 示例
Simple 1: (Player has 光环)

Apply onInteract 光环 to yourself that gives you potion effects when you right click a block that is outlined (one within the 5 block range) OR you are holding an item while you right click.
...
Simple 2: (Player forces the 光环 on nearby entities)

Use sudoskill to force nearby entities with 30 blocks to apply the onInteract 光环 on themselves. Then right click any of the entities to give the potion effects to all other entities in a range of 30 blocks. 
...
Simple 3: (Player has 光环 and forces near by entites to 执行 a skill)

Apply onInteract 光环 to yourself that when you right click a block in your hand OR right click an outlined block (5 block range) it will send all 生物 around you to the moon then after 5 seconds makes them smash back into the ground.
...
Intermediate: (Player has 光环)

Apply onInteract 光环 to yourself and when you right click an outlined block (5 block range) OR right click with an item in your hand cause a circular ring of tremors to grow outwards damaging and sending any hit entities flying backwards.
