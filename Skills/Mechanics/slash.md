## 描述
The Slash meta-技能 can 执行 other skills in a slash-shaped pattern. The exact shape, size, location, rotation and other such elements of the slash itself can be tweaked via the attributes below.
...
| onHitEntitySkill | onHitEntity, ohe, oh | Meta-Skill executed when a point of the slash hits an entity. Only triggered once per entity for every execution of the slash 技能                 |<!--type:Metaskill-->|
...
| Duration       | d           | The amount of ticks the slash should "travel" for. A value of 0 执行 an instantaneous slash                                                                         | 0             |
...
| targetxoffset  | txo, tx     | 目标 X-axis offset                                    | 0             |
| targetyoffset  | tyo, ty     | 目标 Y-axis offset                                    | 0             |
| targetzoffset  | tzo, tz     | 目标 Z-axis offset                                    | 0             |
| 俯仰角(pitch)          |             | The 俯仰角(pitch) rotation                                      | 0             |
| 水平朝向(yaw)            |             | The 水平朝向(yaw) rotation                                        | 0             |
| Roll           |             | The roll rotation                                       | 0             |
| 半径         | r           | The hit 半径                                          | 1             |
| Rotation       | rot         | The rotation of the slash, in the x,y,z format. Same as using 俯仰角(pitch)/水平朝向(yaw)/roll                                                                           | 0,0,0         |
| MatchCasterDirection | matchPlayerDirection, matchDirection, mcd, mpd, md, direction                    | Matches the direction of the slash to 施法者的 facing direction                    | true          |
| directionTowardsTarget | dtt | If the 水平朝向(yaw)/俯仰角(pitch) should be calculated to aim to the 目标 | false      |
| FromOrigin     |             | If the slash should start in the 原点 of the metaskill | false        |
| Hit条件  | 条件, cond, c, oC, hC | List of [Inline 条件](/Skills/Inline-条件) that an entity must met to be hit by the slash 技能            |<!--type:条件-->|
