## 描述
The Polygon meta-技能 can 执行 other skills in a polygon-shaped pattern. The exact number of vertices, the scale, the duration and other such elements of the polygon itself can be tweaked via the attributes below.
...
| onHitEntitySkill | onHitEntity, ohe, oh | Meta-Skill executed when the polygon hits an entity. Only triggered once per entity inside the polygon area for every execution of the polygon 技能 |<!--type:Metaskill-->|
...
| targetxoffset  | tx, txo     | 目标 X-axis offset                                          | 0       |
| targetyoffset  | ty, tyo     | 目标 Y-axis offset                                          | 0       |
| targetzoffset  | tz, tzo     | 目标 Z-axis offset                                          | 0       |
...
| 半径         | r           | The hit 半径                                          | 1             |
| Rotation       | rot         | The rotation of the polygon, in the x,y,z format. Same as using 俯仰角(pitch)/水平朝向(yaw)/roll                                                                           | 0,0,0         |
| MatchCasterDirection | matchPlayerDirection, matchDirection, mcd, mpd, md, direction                    | Matches the direction of the polygon to 施法者的 facing direction                  | true         |
| directiontowardstarget | dtt | 是否 the 水平朝向(yaw)/俯仰角(pitch) should be calculated so that the drawn shape aims to the 目标 | false |
| FromOrigin     |             | If the polygon should start in the 原点 of the metaskill | false      |
| Hit条件  | 条件, cond, c, oC, hC | List of [Inline 条件](/Skills/Inline-条件) that an entity must met to be hit by the polygon 技能          |<!--type:条件-->|
