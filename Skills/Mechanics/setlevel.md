## 描述
Causes the casting 生物 to change its level.
...
## 示例
This will set the 生物 level to 3 when it spawns
```yaml
    - setlevel{a=set;l=3} @self ~onSpawn
```
##
This will increase the 生物 level by 1 each time it kills a player
```yaml
    - setlevel{a=add;l=1} ~onKillPlayer
```
##
This will set the level of the 生物 to a random value between 1 and 5 when it spawns, plus the score of the `__GLOBAL__` fake player from the "testlevel" 计分板.
```yaml
