...
Only one spell 可以 cast at a time, and which runs as an aura on the
...
```
```yaml
# This 将会 cast once the duration has elapsed
FrostBlast-Cast:
  Skills:
  - damage{a=20}
  - message{m="MUHAHA, TAKE THAT!"}

# This 将会 cast while the main casting is still in progress
FrostBlast-Tick:
  Skills:
  - particle{p=end_rod;a=4;hs=1;vs=1} @selflocation{y=1}

# This 将会 cast if the aura is somehow removed
FrostBlast-Interrupted:
  Skills:
  - message{m="Tsk, you got me!"}

# This 将会 cast if the original target 对于aura no longer exist
