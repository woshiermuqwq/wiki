...
Creates a mask of blocks around the specified target. The blocks created or changed by this effect will disappear or rather reset to their former state after the specified duration expired. Resetting the blocks at an earlier point in time 可以 achieved by using the blockunmask-effect (see example below).
...
Bukkit material/block names 可以 acquired in-game. Simply hold the block you want to use 对于effect in your hand and then use the command “/itemdb”. This command will output the bukkit-material info along with some other info, including the data value of the block. This command should work on any bukkit/spigot server.


## 属性
| 属性 | 缩写 | 描述 | 默认值 |
|-----------|-----------|----------------------------------------------------------------------|---------|
| material  | mat, m, block, b | The [type of block](https://hub.spigotmc.org/javadocs/bukkit/org/bukkit/Material.html) used 对于blockmask                      | GRAVEL<!--type:Block--> |
