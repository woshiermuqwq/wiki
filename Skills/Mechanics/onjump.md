## 描述
Applies an [光环] to the 目标 that 触发 a skill when they jump.
...
| 属性 | 缩写   | 描述                                                          | 默认值 |
|-----------|-----------|----------------------------------------------------------------------|---------|
| onJump    | oJ, onjumpskill | The Metaskill to 执行 if the 目标 jumps                   |<!--type:Metaskill-->|
| cancelevent | cE      | 是否 to cancel the event when it is triggered                     | false   |
> This 技能 继承 [光环] 技能

## 示例
Apply an onJump 光环 to yourself. Whenever you jump it will put level 1 slow falling on you for 1 second.
```yaml
ApplyAura:
  Skills:
...
```
##
Apply an onJump 光环 to yourself. Whenever you jump teleport to the closest entity within 40 blocks and kill them.
```yaml
ApplyAura:
  Skills:
...
```
##
Apply an onJump 光环 to yourself. Whenever you jump, jump higher into the air, then stop mid air and shoot 弹射物 at every entities within 30 blocks of you repeatedly. 
```yaml
ApplyAura:
  Skills:
