...
技能 and 机制 that use strings. There are some 示例 of usage at the bottom to get you started.
...
| <目标.距离> | 返回 the 距离 在...之间 施法者 and the 机制 目标. If the 目标 不能 be found in the 施法者 世界, 返回 the maximum double 值 |
| <目标.distancesquared> | 返回 the squared 距离 在...之间 施法者 and the 机制 目标. If the 目标 不能 be found in the 施法者 世界, 返回 the maximum double 值 |
...
|:-------------------------:|---------------------------------|------------------------------------------|
| <施法者.var.{VariableName}> | | 返回 the 值 of the 变量 {VariableName} on the 变量 registry of the 施法者 of the 机制 |
| <目标.var.{VariableName}> | | 返回 the 值 of the 变量 {VariableName} on the 变量 registry of the 目标 of the 机制 |
| <世界.var.{VariableName}> | | 返回 the 值 of the 变量 {VariableName} on the 变量 registry of the 世界 the 机制 is used in |
...
| <技能.var.伤害-数量> | [~onDamaged] 触发器 <br> [~onAttack] 触发器 <br> [~onBowHit] 触发器 <br> [onDamaged] 机制 <br> [onAttack] 机制 | 返回 the 数量 of 伤害 taken or done |
| <技能.var.伤害-类型> | [~onDamaged] 触发器 <br> [~onAttack] 触发器 <br> [~onBowHit] 触发器 <br> [onDamaged] 机制 <br> [onAttack] 机制 | 返回 the 类型 of 伤害 taken or done, if any |
| <技能.var.伤害-cause> | [~onDamaged] 触发器 <br> [~onAttack] 触发器 <br> [~onBowHit] 触发器 <br> [onDamaged] 机制 <br> [onAttack] 机制 | 返回 the cause of 伤害 taken/done |
| <技能.var.光环-名称> | Every [光环] 机制 | 返回 the 名称 of the 光环 |
| <技能.var.光环-类型> | Every [光环] 机制 | 返回 the 类型 of the 光环 |
| <技能.var.光环-charges> | Every [光环] 机制 | 返回 the 数量 of charges the 光环 has left |
| <技能.var.光环-持续时间> | Every [光环] 机制 | 返回 the remaining 持续时间 of the 光环 |
| <技能.var.光环-持续时间-millis> | Every [光环] 机制 | 返回 the remaining 持续时间 of the 光环, in milliseconds |
| <技能.var.光环-堆叠> | Every [光环] 机制 | 返回 the 数量 of 堆叠 the 光环 has left |
| <技能.var.input> | [onChat] 机制 | 返回 the chat input |
...
| <技能.var.hit-方块-类型> | [raytrace] 机制 | 返回 the 方块 that was hit, or AIR if 没有 |
...
```
> If you 执行 this 元技能 您自己, this will send you a 消息 saying `1 2 1 2` 因为 the 目标 of every 机制 is the 施法者, so the affected registry is 总是 the 施法者 即使 a "目标" 作用域 is used
...
- - If used inside a 机制, the 目标 必须为 a 玩家
...
[onChat]: /技能/机制/onChat
[光环]: /技能/机制/光环
[raytrace]: /技能/机制/raytrace
[raytraceto]: /技能/机制/raytraceto
[onDamaged]: /技能/机制/onDamaged
[onAttack]: /技能/机制/onAttack
[universal 属性]: /技能/机制#universal-属性