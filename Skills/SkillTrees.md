...
SkillTrees are **created each time a 机制 is fired by a 触发器**, and 它是 the place where 技能 scoped [变量] are stored and inside which [元技能] exist
...
## 事件 Data
The skilltree 总是 knows which 事件 triggered it, and 允许 the [cancelevent](/技能/机制/cancelevent) 机制 to be used in any of the 元技能 it calls 为了 cancel it, 只要 this occurs in a synched manner
...
The 实体 casting the 元技能.
  - Can be changed via the use of the [sudoskill] 机制
### `目标`
...
Is first set as the 实体 that triggered the SkillTree initially, and one can fetch this 实体 via the [@触发器] 目标选择器 or 其他 similar 目标选择器. Depending on the [~触发器] used, a [@触发器] 可能不 exist.
  -- The `Trigger` can be changed via the use of the [sudoskill] 机制 `casterastrigger` 属性, which will make the called 元技能 have, as the `Trigger`, the `Caster` of the original 元技能
### `原点`
It the [@原点] of the 元技能. By 默认, 它是 the position of the `Caster`.
  -可以setvia the `origin` [universal 属性]。
  - It is 自动 set in 机制 例如 [弹射物](/技能/机制/弹射物)
### `技能 参数`
