如[技能]维基页面所示，可以通过以下语法从生物执行技能：

```yaml
- mechanic{argument=value} @[targeter] ~on[trigger] [health_modifier] [chance]
```

但如果想要"组合"多个技能，并总体上为生物创建更高级的行为，这种方式并不合适。这可以通过使用元技能来解决。
...
# 什么是元技能？
元技能本质上是一个技能列表，当通过[元技能]调用元技能时执行。
...
配置好元技能后，你可以使用[元技能]（如 [Skill]）从生物或元技能中执行它：
```yaml
#生物文件
ExampleMob:
  Type: ZOMBIE
...
## Skill
不要与 [Skills](#skills) 混淆。此选项允许元技能在触发时执行另一个元技能的技能。
```yaml
example1:
  Conditions:
  - night true
...
此执行的行为非常特殊：
- 另一个元技能的技能在当前元技能的 [Skills](#skills) 之前执行
...
## Skills
元技能的真正核心。这是元技能触发时将执行的技能列表。其他[元技能]也可以在此使用，允许元技能触发其他元技能。
...
延迟也可以在此使用，允许用户在延迟技能之后的列表中的每个技能之间设置延迟。
...
技能通常从列表中的第一个到最后一个依次执行。如果使用了[元技能]，其技能将在原始元技能的技能继续执行之前执行。如果被调用的元技能中包含延迟，则原始元技能的技能继续执行，而被调用元技能中的技能将在延迟之后执行。

![](https://i.imgur.com/ZiHWeBQ.png)
> 在这张图片中，展示了上述行为的示例。如果执行 `ExampleSkill_First`，技能将按照编号从 mechanic1 到 mechanic9 的顺序执行，mechanic7、mechanic8 和 mechanic9 之间有 20 tick 的延迟。

# 目标继承与覆盖
元技能的一个特性是能够"记住"调用它的[元技能]传递给它的目标。此行为有许多用途和应用。

## 继承
元技能中没有自己的目标选择器的技能将**继承**这些目标，并以它们为目标。
...
ExampleSkill:
  ...
  - ignite
```
在此示例中，`ignite` 技能没有目标选择器，因此它"继承"了元技能中使用的目标选择器，转而瞄准那些实体，使其点燃 10 格半径内的所有玩家。
...
内联元技能中的技能不能用普通方式注释掉以禁用它们，因为这会导致整个元技能停止工作。必须使用 `<#>` 或 `<&nm>` 等字符在技能前以获得相同效果。

内联元技能语法可用在所有需要普通元技能内部名称的元技能中。
...
```
> 在此示例中，你可以看到如何在内联元技能中注释掉一个技能，以及如何在调用元技能时正常使用普通属性。
...
"技能参数"系统会将 **skill/metaskill** 技能中的**任何**选项（除其专属选项外）沿着技能树传递，你可以在后面引用它们。如果后续技能传递了相同的参数，它会覆盖之前的值。这些参数可以在任何支持占位符的地方使用。

> 技能参数可以从 [skill](/Skills/Mechanics/skill) 和 [variableskill](/Skills/Mechanics/variableskill) 技能中使用。
