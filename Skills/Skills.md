...
技能由[技能机制][机制]（或称为"基础技能"）组成，这些是 MythicMobs 自带的基础技能。每个技能机制在生物的 **Skills** 部分中调用。让我们来看一个例子：
...
每个技能机制以列表形式分配给生物。但一个真正的机制是什么样子？来看一个实际例子：
```yaml
FieryZombie:
  Type: ZOMBIE
  Display: 'Fiery Zombie'
...
技能的第一个、也是最重要的部分，是机制。它定义了你想做什么，是你正在执行的基础技能。它可能是造成伤害、点燃目标、或者释放闪电。[有大量不同的机制][机制]可供使用，主要分为两种类型：以实体为目标的机制，和以位置为目标的机制。有些可以同时针对两者，也有些两者都不需要。

大多数机制也有参数。它们紧跟在机制名称之后，用花括号 {} 包裹。每个参数用分号（;）分隔。每种机制都有自己独特的参数，全部可以在[机制]维基页面查阅。
...
设置一个机制很简单：确定你想使用的机制，然后填入你需要的选项。大多数选项都有默认值且是可选的。所以如果你想把人点燃 5 秒，你可以使用 ignite 机制并将 "ticks" 选项设为 100（每秒 20 tick）：
...
目标选择器直接跟在机制后面的技能行中，始终以 @ 符号为前缀。有些还可以有自己的选项，同样用花括号跟在目标选择器后面。
