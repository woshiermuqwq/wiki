...
-   修复了 some console 错误s caused by players having recently changed
    worlds
-   修复了 LegacySkills failing due to async 错误s
...
-   Changed a large portion of mob skills to run async for a huge
    performance boost
-   修复了 ThreatTables 不生效的问题 on some versions of Spigot
...
-   修复了 some crippling 漏洞s that somehow wormed their way into 2.1.6
...
-   添加了 future 支持 for MythicChampions plugin with "champions-exp
    \#" drop type
...
-   修复了 an 空指针异常 with MaxCombatDistance option
...
-   修复了 some other minor 漏洞s with spawners

### 2.1.4

#### Bug Fixes / Other

-   修复了 some compatibility 问题s with 1.7
-   Fixed SendTitle skill not targeting players
-   Fixed ThreatTables breaking when a mob targets a non-player that
    dies.
-   修复了 KillAll command 不生效的问题 with radius
...
-   修复了 a critical 漏洞 with equipment not equipping
-   修复了 a few other 漏洞s with drop tables
...
-   修复了 compatibility 问题s with a couple other plugins

### 2.1.1

#### Bug Fixes / Other

-   修复了 particles 不生效的问题 on non-1.8 servers
-   修复了 some compatibility 问题s with EchoPet
-   修复了 display 错误s with ParticleBox effect
-   修复了 display 错误s with ParticleSphere effect
...
-   添加了新的 per-world level scaling 选项s to config.yml for scaling
    mob level based on distance from the spawn
-   添加了新的 default LevelModifier 选项s to config.yml for
...
-   修复了 colors 不生效的问题 in player disguises
