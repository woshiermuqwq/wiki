...
## 技能（Mechanics）
[技能 Wiki](/Skills/Mechanics)

技能是为你的生物添加有趣且独特能力的主要方式。最基本的使用方式是将技能添加到生物的 Skills 部分，并配合触发器和目标选择器，但也可以将其添加到元技能中。大多数技能都有可使用的属性列表，用于修改其工作方式。
...
目标选择器告诉技能以谁为目标。例如，如果你使用 giveitem 技能，目标选择器将决定把物品给谁。
...
触发器告诉生物何时激活其技能和能力。触发器必须以 `~` 开头，并放在目标选择器之后。触发器只能直接写在生物上，写在元技能中将不起作用。示例中，`~onTimer:20` 是触发器，告诉技能每 20 ticks（1 秒）运行一次。
...
## 属性（Attributes）
属性是你可以传递给技能/目标选择器/条件的信息，用于自定义其工作方式。在此示例中，`cd=10` 和 `i=DIAMOND` 是属性，告诉技能要给予什么物品以及冷却时间为 10 秒。属性使用 `;` 符号分隔，且必须在你设置的值之前加上 `=`。

`- giveitem{i=DIAMOND;cd=10} @NearestPlayer ~onTimer:20`

<details><summary>属性表</summary>
在每个技能的 Wiki 页面中，你会看到一个表格，列出了所有可用的属性（如有）及相关信息。
例如：
...
这告诉你该技能有 3 个不同属性：`属性1`、`属性2` 和 `属性3`。每个属性都有对应的功能描述和一个"默认值"，如果未显式指定，将应用默认值。有些属性还有"别名"，就是你设置同一属性时可以使用的不同名称。
</details>

<details><summary>属性继承</summary>
有时你会在属性部分看到*类似*这样的文字：

> 此技能继承 [Damage](/Skills/Mechanics/Damage) 技能的每一个*可继承*属性

这意味着除了 Wiki 页面本身的属性外，所指示技能中的属性也会被使用（除非标记为"不可继承"）。例如，用作示例的 Damage 技能只有一个不可继承的属性（`amount`），而其他所有属性都被继承。
</details>
...
技能是 MythicMobs 的基本功能，它让你能够将多个技能组合成一个，使你能够创建带有多种效果和视觉表现的有趣攻击和功能。
...
变量是一个用于存储信息的便捷系统。之后你可以在各种其他地方使用这些信息，例如技能和条件中的占位符。
